...
func (fs *filesystem) getChildLocked(ctx context.Context, parent *dentry, name string, ds **[]*dentry) (*dentry, error) {
	if child, ok := parent.children[name]; ok {
		// If child is enabled, we should check again whether the file
		// and the corresponding Merkle tree are as expected, in order
		// to catch deletion/renaming after the last time it's
		// accessed.
		if child.verityEnabled() {
			...
			// The file was previously accessed. If the file does
			// not exist now, it indicates an unexpected
