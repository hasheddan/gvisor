...
func (ep *endpoint) newHandshake() *handshake {
	h := &handshake{
		ep:          ep,
		active:      true,
		rcvWnd:      seqnum.Size(ep.initialReceiveWindow()),
...
func (ep *endpoint) newPassiveHandshake(isn, irs seqnum.Value, opts *header.TCPSynOptions, deferAccept time.Duration) *handshake {
	h := ep.newHandshake()
