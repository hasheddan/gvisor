...
const (
	...
	minimumHeaderReserve = header.EthernetMinimumSize + header.IPv6MinimumSize
)
...
func compareFragments(packets []*stack.PacketBuffer, sourcePacket *stack.PacketBuffer, mtu uint32, wantFragments []fragmentInfo, proto tcpip.TransportProtocolNumber) error {
	...
	for i, fragment := range packets {
		...
		if got := fragment.AvailableHeaderBytes(); got != header.EthernetMinimumSize {
...
func TestFragmentationWritePacket(t *testing.T) {
	...
	for _, ft := range fragmentationTests {
		t.Run(ft.description, func(t *testing.T) {
			pkt := testutil.MakeRandPkt(ft.transHdrLen, minimumHeaderReserve, []int{ft.payloadSize}, header.IPv6ProtocolNumber)
			source := pkt.Clone()
...
func TestFragmentationWritePackets(t *testing.T) {
	...
	tinyPacket := testutil.MakeRandPkt(header.TCPMinimumSize, minimumHeaderReserve, []int{1}, header.IPv6ProtocolNumber)
