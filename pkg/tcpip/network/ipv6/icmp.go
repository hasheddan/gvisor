...
func (e *endpoint) handleICMP(pkt *stack.PacketBuffer, hasFragmentHeader bool) {
	...
	src := iph.SourceAddress()
	dst := iph.DestinationAddress()
	...
	if got, want := h.Checksum(), header.ICMPv6Checksum(h, src, dst, payload); got != want {
		received.Invalid.Increment()
		return
	}
	...
	case header.ICMPv6NeighborSolicit:
		...
		// solicitations.
		unspecifiedSource := src == header.IPv6Any
		if len(sourceLinkAddr) == 0 {
			if header.IsV6MulticastAddress(dst) && !unspecifiedSource {
				received.Invalid.Increment()
				return
			}
			...
			e.nud.HandleProbe(src, dst, header.IPv6ProtocolNumber, sourceLinkAddr, e.protocol)
		} else {
			e.linkAddrCache.AddLinkAddress(e.nic.ID(), src, sourceLinkAddr)
		}
		...
		if unspecifiedSource && !header.IsSolicitedNodeAddr(dst) {
			received.Invalid.Increment()
			return
		}
	...
	case header.ICMPv6EchoRequest:
		...
		localAddr := dst
		if header.IsV6MulticastAddress(dst) {
			localAddr = ""
		}

		r, err := e.protocol.stack.FindRoute(e.nic.ID(), localAddr, src, ProtocolNumber, false /* multicastLoop */)
		if err != nil {
			// If we cannot find a route to the destination, silently drop the packet.
			return
		}
	...
	case header.ICMPv6RouterSolicit:
		...
		if len(sourceLinkAddr) != 0 {
			...
			if e.nud != nil {
				...
				e.nud.HandleProbe(src, dst, ProtocolNumber, sourceLinkAddr, e.protocol)
			}
		}
	...
	case header.ICMPv6RouterAdvert:
		...
		if len(sourceLinkAddr) != 0 && e.nud != nil {
			e.nud.HandleProbe(routerAddr, dst, ProtocolNumber, sourceLinkAddr, e.protocol)
		}
